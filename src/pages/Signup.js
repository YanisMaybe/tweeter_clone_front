                ...
                <div className="signup">
                    ...
                    <div className="content">
                        <form onSubmit={e => {
                            ...
                            axios.post("https://tweeter-clone-back-22.onrender.com/auth/createUser", data).then(res => {
                                if (res.data) {
                                    if (res.data.error) {
                            ...
                            <div className="userName contentInputs">
                                <label>Nom d'utilisateur</label>
                                <input onChange={() => setPonce(true)} onClick={onClickInInput} className="zc223dsDsq input" type="text" />
                            </div>
                            <div className="email contentInputs">
                                <label>Votre email</label>
                                <input onChange={() => setPonce(true)} onClick={onClickInInput} className="zc223dsDsq input" type="email" />
                            </div>
                            <div className="passwordInput contentInputs">
                                <label>Mot de passe</label>
                                <input autoComplete={false} onClick={onClickInInput} className="zc223dsDsq" type="password" />
                            </div>
                            <div className="submitZone">
                                {inputs[0] === "" || inputs[1] === "" || !inputs[0] || !inputs[1] ? <input className="btn disabledBtn" type="submit" value="S'inscrire sur twitter" /> : null}
                                {inputs[0] !== "" && inputs[1] !== "" && inputs[0] && inputs[1] ? <input className="btn" type="submit" value="S'inscrire sur twitter" /> : null}
                            </div>
                        </form>
                    </div>
                    <div className="footer" onClick={onLeaveInInput}>
                        <NavLink style={{ marginTop: '25px' }} to="/login" className="navAuth">Se connecter a Tweetter</NavLink>
                    </div>
                </div>
            </div>}
        </>
